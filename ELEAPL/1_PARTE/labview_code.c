...
int main(void)
{
	...
	while (1)
	{
		if (timer_flag == 1) //De 500ms em 500ms..
		{
			...
			unsigned int ad_mean = (unsigned int) (sum/64);//Calcular media das leituras do ad (0 <= ad_mean <= ad_meam)
			float ad_converted = (float) (ad_mean * 20.0 / 1023.0) + 17.0;//Converter em temperatura a variavel ad_mean
