...
char do_mode; // Fun?ao a fazer dentro do modo de funcionamento especificico
char msg[50]; // Mensagem a enviar para o Putty
volatile USARTRX_st rxUSART = {0, 0, 0, 0}; // Inicializar Struct de comunicacao de dados a 0
...
volatile unsigned int contador_subidas = 0;
volatile unsigned char timer_passos = 0; // Contador de 25ms para o motor passo a passo
volatile signed char n_passos = 0; // Numero de Passo atual
...
signed char posicao_atual = 0; // Posi?ao atual comparado ? referencia
...
char temp_str[4]; // Vetor de Chars para montar o N? de passos enviado pelo PC
unsigned char contador_str = 0; // Contador para circular e  montar o vetor de Chars que ira ser transformado em numero de passos a utilizar
volatile unsigned int rps_carrier = 0;
volatile unsigned int rps = 0;
volatile unsigned int contador_1s;
volatile unsigned char flagrps = 0;



unsigned char ler_valor_asm(); // Funcao Assembly
void handle_receiver_buffer(); // Tratamento do buffer
void calcular_rps(unsigned int rps_temp);
void AtualizarDisplays(unsigned char value, signed char pos_atual, unsigned int rps_t); // Atualizar Displays
...
void sw4(); // SW4
void sw5(); // SW5
void duty_analog(); // Fun??o para calcular a m?dia do potenciometro para meter no OCR.
void SendString(char *msg); // Fun?ao enviar String
unsigned char calculo_OCR(unsigned char percentagem); // Fun??o c?lculo OCR a partir da percentagem.
void motorpp_negativo(void); // Fun??o para andar no sentido positivo do motor PP.
void motorpp_positivo(void);  // Fun??o para andar no sentido negativo do motor PP.
...
ISR(TIMER0_COMP_vect)
{
	...
	AtualizarDisplays(ocr_percentagem, posicao_atual, rps);
	if (flag_stop250ms == 1)
	{
		contador_250ms++;
	}
	...
	if (function_mode == 'v' || function_mode == 'V'){
	contador_1s ++;
	if (contador_1s >= 200)
	{
		rps_carrier = contador_subidas;
		contador_subidas = 0;
		contador_1s = 0;
		flagrps = 1;
	}
...
void motorpp_positivo(void)
{
	...
	PORTE = array_passes_p[passo_funcs_positivo];
...
void motorpp_negativo(void)
{
	...
	PORTE = array_passes_n[passo_funcs_negativo];
...
void AtualizarDisplays(unsigned char value, signed char pos_atual, unsigned int rps_t)
{
	contador3++;
	if (contador3 > 3)
	{
	...
	if (contador3 == 3)
	{
		...
		else if (function_mode == 'v' || function_mode == 'V')
		{
			PORTC = 0b11000001;
		}
		else
...
void handle_receiver_buffer(void)
{
	if (rxUSART.receiver == 1)
	{
		...
		else
		{
			...
			|| rxUSART.receiver_buffer == 'a' || rxUSART.receiver_buffer == 'A' || rxUSART.receiver_buffer == 'm' || rxUSART.receiver_buffer == 'M'
			|| rxUSART.receiver_buffer == 'v' || rxUSART.receiver_buffer == 'V')
			{
...
int main(void)
{
	...
	while (1)
	{
		...
		else if (function_mode == 'v' || function_mode == 'V')
		{
			if (flagrps == 1)
			{
				calcular_rps(rps_carrier);
			}
			switch (do_mode)
			{
				case 'b':
				case 'B':
				sprintf(msg, "RPS: %d\n Duty: %d %% \r\n", rps,ocr_percentagem); // Mexer e colocar RPS.
				SendString(msg);
				do_mode = 0;
				break;
				default:
